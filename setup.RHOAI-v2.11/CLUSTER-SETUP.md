...
```sh
oc apply -f setup.RHOAI-v2.11/mlbatch-priorities.yaml
```
...
```sh
oc patch deployment -n scheduler-plugins --type=json --patch-file setup.RHOAI-v2.11/coscheduler-priority-patch.yaml scheduler-plugins-controller
...
```sh
oc apply -f setup.RHOAI-v2.11/mlbatch-dsci.yaml
```
...
```sh
oc apply -f setup.RHOAI-v2.11/default-flavor.yaml
```
...
```sh
oc apply -f setup.RHOAI-v2.11/mlbatch-edit-role.yaml
```
